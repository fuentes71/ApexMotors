...
// Constant lookup — module scope so it isn't rebuilt on every render (BAS-63).
...
export default function DashboardPage() {
  ...
  // All ~20 derived aggregates recompute only when the underlying data or the
  // period changes — not on every ephemeral UI re-render (mobile menu, modals).
  // Each result is a stable reference, so Recharts doesn't re-layout needlessly.
  const {
    netBalance,
    totalVehicleProfit,
    totalFixed,
    prevNetBalance,
    prevTotalVehicleProfit,
    prevTotalFixed,
    soldVehiclesCount,
    inStockVehiclesCount,
    avgProfit,
    pieData,
    barData,
    avgTicket,
  ...
  }, [vehicles, fixedExpenses, startMonth, endMonth, employees]);
